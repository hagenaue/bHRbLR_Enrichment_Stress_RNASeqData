#Taking a quick peek at Angela's LR social defeat and environmental enrichment RNA-Seq data (Nacc)
#Megan Hagenauer
#September 8, 2019
...
#Note - I quickly tweaked this in Excel so that Cort would read in as numeric (it had "_" written as NAs originally)

#Sample_MetaData<-read.delim("Angela_Sample_MetaData.txt", sep="\t", header=T, stringsAsFactors = F)

Sample_MetaData<-read.delim("Angela_Sample_MetaData_Updated20200910.txt", sep="\t", header=T, stringsAsFactors = F)
#Note - I discovered that the date of dissection had a formatting error in the data so that several subjects had an extra space (" 17/1/2019" needed to be changed to "17/1/2019" and " 21/2/2019" needed to be changed to "21/2/2019")
...
# $ date_of_dissection       : chr  "16/1/2019" "17/1/2019" "13/3/2019" "16/1/2019" ...
# $ hemisphere_dissected     : chr  "right" "left" "right" "left" ...
# $ date_of_RNA_extraction   : chr  "28.2.2019" "1.3.2019" "13.3.2019" "28.2.2019" ...
# $ RNA_conc                 : num  80.1 61.6 56.7 102.6 103.9 ...
# $ ratio_260_280            : num  2.06 2.13 2.01 2.03 2.02 2.06 2.07 2.01 2.01 2.05 ...

#Old version of the analysis (from 10/2020)
#setwd("~/Documents/Microarray Gen/Angela_HRLR_EE_Stress/NAcc_Remove2outliers")

#Sample_BehavHormonalData<-read.csv("HRLR_EE_Stress_AllBehavData_forR_withNewCORTOxytIL6_SI_SDScoresFixed_FixedFormatIDs_TimeOnTop.csv", header=T, stringsAsFactors = F)
#Note: the open field and USV data within this dataset are both a work in progress and any analyses including them will need to be re-done.
...
# Pcdh17
# RT1-T24-3
#apparently teh RT-1s are peptide-antigen binding and orthologous to HLA-E. interesting.
...
# pdf("Oprm1_Prettier_vs_Group_v3.pdf", width=5, height=7)
# boxplot(GeneY~GroupingVariable, ylab="Oprm1 Log2 Cpm", col=c("red", "pink", "red", "pink"))
# stripchart(GeneY~GroupingVariable, vertical = TRUE,  method = "jitter", add = TRUE, pch = c(20, 1, 17, 2), cex=2, cex.axis=10, cex.lab=10, col = 'black')
# dev.off()
# 
# GroupingVariable<-c(rep("No Stress", 8), rep("Stress", 8), rep("No Stress", 7), rep("Stress", 6), rep("No Stress", 2), rep("Stress", 4), rep("No Stress", 6), rep("Stress", 5))
# 
# cbind(GroupingVariable, Sample_MetaData_NACC_NoOutliers_Ordered$Treatment_group)
# 
# pdf("Oprm1_Prettier_vs_Group_v4.pdf", width=3, height=7)
# boxplot(GeneY~GroupingVariable, ylab="Oprm1 Log2 Cpm", col=c("red", "pink"))
# stripchart(GeneY~GroupingVariable, vertical = TRUE,  method = "jitter", add = TRUE, pch = c(20, 1), cex=2, cex.axis=10, cex.lab=10, col = 'black')
# dev.off()


table(Sample_MetaData_NACC_NoOutliers_Ordered$Treatment_group)
...
GMT_ForRats<-gmtPathways("CombinedGMT_C2_C5_JustTraditional_CellType_20170928.gmt.txt")
names(GMT_ForRats)
GMT_ForRats[[1]]
#It might be worthwhile to update this file - it is a little old.
#...but I checked their website and it looks like they haven't updated it since our last download. 

setwd("~/Documents/Microarray Gen/Angela_HRLR_EE_Stress/NAcc_Remove2outliers")

colnames(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect)

NAcc_EE_Betas_forGSEA<-NAcc_LimmaResults_EEbySD_RNAConcExtractDissect[,4]
str(NAcc_EE_Betas_forGSEA)
names(NAcc_EE_Betas_forGSEA)<-toupper(NACC_RNASeq_Log2_Annotated$gene_symbol)
...
# -2.471693      -2.425082      -2.256903      -2.154923      -2.124847      -2.092887
#sanity check: matches our output sorted in excel

temp1<-fgsea(GMT_ForRats, NAcc_EE_Betas_forGSEARanked, nperm=10000, minSize = 15, maxSize = 500)
str(temp1)

temp1$leadingEdge<-vapply(temp1$leadingEdge, paste, collapse= ",", character(1L))

write.csv(temp1, "NAcc_EE_FGSEAResultswCellType2.csv")

#Interesting - protocadherin pathway isn't popping up near the top, maybe because it is a mixture of direction of effect.
#Most of the top pathways are related to microtubules/neuronal processes


colnames(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect)

NAcc_SD_Betas_forGSEA<-NAcc_Coefficients_EEbySD_GenRNAConcExtract[,5]
str(NAcc_SD_Betas_forGSEA)
names(NAcc_SD_Betas_forGSEA)<-toupper(NACC_RNASeq_Log2_Annotated$gene_symbol)
sum(duplicated(names(NAcc_SD_Betas_forGSEA)))
#[1] 908
sum(is.na(names(NAcc_SD_Betas_forGSEA)))
#[1] 766
length(NAcc_SD_Betas_forGSEA)
#[1] 17765

NAcc_SD_Betas_forGSEA<-NAcc_SD_Betas_forGSEA[is.na(names(NAcc_SD_Betas_forGSEA))==F]
length(NAcc_SD_Betas_forGSEA)
#[1] 16999
sum(is.na(names(NAcc_SD_Betas_forGSEA)))
#[1] 0
...
# AABR07040887.1           TP73         CFAP44        CFAP206   LOC102554658          CLIC6 
# -2.246933      -2.007722      -1.924679      -1.921888      -1.756215      -1.752158 
#sanity check: matches our output sorted in excel

temp1<-fgsea(GMT_ForRats, NAcc_SD_Betas_forGSEARanked, nperm=10000, minSize = 15, maxSize = 500)
str(temp1)

temp1$leadingEdge<-vapply(temp1$leadingEdge, paste, collapse= ",", character(1L))

write.csv(temp1, "NAcc_SD_FGSEAResultswCellType2.csv")
#Interesting - many of the same pathways appear to be altered and showing the same direction of effect.
#Lots of microtubule & neuronal processes related pathways again.


colnames(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect)


#Switching to the version that lacked the interaction term:

colnames(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect)

NAcc_EE_Betas_forGSEA<-NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect[,4]
str(NAcc_EE_Betas_forGSEA)
names(NAcc_EE_Betas_forGSEA)<-toupper(NACC_RNASeq_Log2_Annotated$gene_symbol)
sum(duplicated(names(NAcc_EE_Betas_forGSEA)))
#[1] 908
...
# -2.235930      -2.168639      -2.025907      -1.861531      -1.845408      -1.836028 
...
write.csv(temp1, "NAcc_EE_noInteraction_FGSEAResultswCellType2.csv")


NAcc_SD_Betas_forGSEA<-NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect[,5]
str(NAcc_SD_Betas_forGSEA)
names(NAcc_SD_Betas_forGSEA)<-toupper(NACC_RNASeq_Log2_Annotated$gene_symbol)
sum(duplicated(names(NAcc_SD_Betas_forGSEA)))
#[1] 908
...
# -3.376474      -2.785706      -2.044384      -1.585838      -1.288447      -1.214261 
...
write.csv(temp1, "NAcc_SD_noInteraction_FGSEAResultswCellType2.csv")
#Interesting - many of the same pathways appear to be altered and showing the same direction of effect.
#Lots of microtubule & neuronal processes related pathways again.
...
#Tmem144 is an HRvsLR DE gene (Adult FDF<0.05 - also DE in many other models) with sig effect of SD and nominal effect of EE
...
#This seems like a lot. Maybe we should do some sort of enrichment analysis/Venn diagram.
#Also: I would like to know how often the effects are a reversal of HR/LR differences


#Overlap analysis:

sum(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.01, na.rm=T)
#[1] 586
sum(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.05, na.rm=T)
#[1] 1666
sum(is.na(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE)
#[1] 13836
...
sum(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2[is.na(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE]<0.01, na.rm=T)
#[1] 130
sum(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2[is.na(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE]<0.05, na.rm=T)
#[1] 552
length(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2[is.na(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE & is.na(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2)==F])
#[1] 13836

130/13836
#[1] 0.009395779
#Approximately random chance

552/13836
#[1] 0.03989592
#Less than random chance...

sum(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.BH<0.05 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.01, na.rm=T)
#[1] 2

NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$gene_symbol[which(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.BH<0.05 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.01)]
#[1] "Uhrf1" "Etv4" 
#Uhrf1 has a reasonable EE effect of (p=0.0028) - drops to a weak EE effect without the interaction term (p=0.04) - down with enrichment, up in LR
#Etv4 has a reasonable EE effect of (p=0.001) - drops to a pretty weak EE effect without the interaction term (p=0.02) - down with enrichment, down in SD, up in HR

sum(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.01 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.01, na.rm=T)
#[1] 9

NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$gene_symbol[which(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.01 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.01)]
#[1] "Ccl19"   "Grhl3"   "Ifnlr1"  "Fos"     "Uhrf1"   "Egfr"    "Cbx1"    "Etv4"    "Pcdhga1"

sum(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.05 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.05, na.rm=T)
#[1] 78

9/130
#[1] 0.06923077
#So 7% of the nominally significant EE results (p<0.01) are also nominally significant in the HR/LR meta-analysis
#This is maybe 1.5x what you might expect to see by chance. 

78/552
#[1] 0.1413043
#So 14% of the nominally significant EE results (p<0.05) are also nominally significant in the HR/LR meta-analysis
#Which is actually what you might expect due to random chance (since 12% of the genes in the HR/LR meta-analysis are nominally significant)


sum(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2[is.na(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE]<0.01, na.rm=T)
#[1] 314
sum(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2[is.na(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE]<0.05, na.rm=T)
#[1] 1288
length(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2[is.na(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE & is.na(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2)==F])
#[1] 13836
...
NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$gene_symbol[which(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.BH<0.05 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.01)]
#[1] "Tmem144"

#Tmem144 has a reasonable sd effect (p=0.00058) that is not significant if we remove the interaction term, up in LR, up in SD and EE.

sum(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.01 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.01, na.rm=T)
#[1] 16

NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$gene_symbol[which(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.01 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.01)]
# [1] "Rab32"   "Htra1"   "Tmem144" "Pias3"   "Gcfc2"   "Rps4y2"  "Harbi1"  "Slx4ip"  "Faxc"    "Ccl19"  
# [11] "Ddi2"    "3-Sep"   "Acad11"  "Alb"     "Gfpt2"   "Pcdhga1"

sum(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.05 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.05, na.rm=T)
#[1] 169

16/314
#[1] 0.05095541
#So 5% of the nominally significant EE results (p<0.01) are also nominally significant in the HR/LR meta-analysis. This is what you would expect based on random chance (since 5% of the HR/LR meta-analysis results are nominally sig at p<0.01)

169/1288
#[1] 0.1312112
#So 13% of the nominally significant EE results (p<0.05) are also nominally significant in the HR/LR meta-analysis
#Which is actually what you might expect due to random chance (since 12% of the genes in the HR/LR meta-analysis are nominally significant)

sum(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.01 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.01 & is.na(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE, na.rm=T)
#[1] 38

sum(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.05 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.05 & is.na(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE, na.rm=T)
#[1] 179


#Proportion of EE genes:
...
#Both of those are definitely an enrichment


sum(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.01 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.01 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.01, na.rm=T)
#[1] 2

sum(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.05 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.05 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.05, na.rm=T)
#[1]  31

2/38
#[1] 0.05263158
#This is what you would expect based on random chance (since 5% of the HR/LR meta-analysis results are nominally sig at p<0.01)

NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$gene_symbol[which(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.01 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.01 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.01)]
#[1] "Ccl19"   "Pcdhga1"

31/179
#[1] 0.1731844
#This is a little more than what you would expect based on random chance...

#So maybe not so exciting. 

NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$gene_symbol[which(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.05 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.05 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.05)]
# [1] "Synj2"     "Lipt2"     "Acsm3"     "Pygm"      "Gapt"      "Nhlrc3"    "Tmem144"   "Faxc"     
# [9] "Ccl19"     "Frmpd1"    "Tspan1"    "Grhl3"     "Ifnlr1"    "Klhdc7a"   "Ptchd1"    "Acot4"    
# [17] "Acot6"     "Papola"    "Tdg"       "Chordc1"   "Barx2"     "Hsd17b13"  "Art3"      "Egfr"     
# [25] "Slc13a5"   "Etv4"      "Hist1h2bk" "Wwc2"      "Pcdhb5"    "Pcdhga1"   "Vars2"      

NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$gene_symbol[which(NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.05 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.05 & NAcc_LimmaResults_EEbySD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.BH<0.05)]
#[1] "Tmem144" "Tdg"     "Etv4" 

#Both of the Tdg effects are super weak (p=0.03 for both, small effect sizes) -up in EE and SD, up in LR
...
#Visually it seems like there is less overlap than before. Let's quantify it:

#Overlap analysis:

#See calculations above for HR/LR meta-analysis only stats

sum(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2[is.na(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE]<0.01, na.rm=T)
#[1] 156
sum(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2[is.na(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE]<0.05, na.rm=T)
#[1] 705
length(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2[is.na(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE & is.na(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2)==F])
#[1] 13836
...
NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$gene_symbol[which(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.BH<0.05 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.01)]
#[1] "Tubg1"

#Reasonable effect of EE in the model that doesn't include an interaction term (p=0.0009), drops to a trend if the model does inclue an interaction term. Up in EE, up in bLR

sum(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.01 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.01, na.rm=T)
#[1] 11

NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$gene_symbol[which(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.01 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.01)]
#[1] "Pex11a" "Mrpl43" "Gsap"   "Grhl3"  "Arr3"   "Fos"    "Acad11" "Egfr"   "Tubg1"  "Naa50"  "Mcoln1"

sum(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.05 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.05, na.rm=T)
#[1] 101
...
sum(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2[is.na(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE]<0.01, na.rm=T)
#[1] 247
sum(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2[is.na(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE]<0.05, na.rm=T)
#[1] 985
length(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2[is.na(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE & is.na(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2)==F])
#[1] 13836
...
NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$gene_symbol[which(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.BH<0.05 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.01)]
#[1] "Slc19a3" "Prss55"

#Slc19a3 has a reasonable effect in the model without an interaction term (p=0.0012), becomes more moderate in the version that doesn't have an interaction term (p=0.029). Down in SD, up in LR
#Prss55
#has a reasonable effect in the model without an interaction term (p=0.009), becomes more moderate in the version that doesn't have an interaction term (p=0.0205). Down in SD, up in LR

sum(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.01 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.01, na.rm=T)
#[1] 7

NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$gene_symbol[which(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.01 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.01)]
#[1] "Rab32"   "Ntrk1"   "Ddi2"    "Slc19a3" "Alb"     "Glul"    "Prss55" 

sum(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.05 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.05, na.rm=T)
#[1] 127

7/247
#[1] 0.02834008
#So 2% of the nominally significant EE results (p<0.01) are also nominally significant in the HR/LR meta-analysis. This is less than what you would expect based on random chance (since 5% of the HR/LR meta-analysis results are nominally sig at p<0.01)
...
sum(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.01 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.01 & is.na(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE, na.rm=T)
#[1] 8

sum(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.05 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.05 & is.na(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp)==FALSE, na.rm=T)
#[1] 68

#Much less overlap between SD and EE now in the model that doesn't include an interaction term.
...
#Only a moderate enrichment now.
...
sum(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.01 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.01 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.01, na.rm=T)
#[1] 0

sum(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.05 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.05 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.05, na.rm=T)
#[1] 9
...
9/68
#[1] 0.1323529
#This is exactly what you would expect due to random chance.

#So maybe not so exciting. 

NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$gene_symbol[which(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.05 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.05 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.rawp<0.05)]
# [1] "Atp4a"     "Pex11a"    "Gapt"      "Pdlim5"    "Slx4ip"    "Zmynd12"   "Slc25a32"  "Acad11"   
# [9] "Hist1h2bk"  


NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$gene_symbol[which(NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$EnrichmentEE.2<0.05 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$SocialDefeatSD.2<0.05 & NAcc_LimmaResults_onlyEESD_RNAConcExtractDissect_vsMetaAnalysis$Adult_AdultMeta.BH<0.05)]
#character(0)
...
##Scrap:
##Digging though Fan's preliminary results from 2019:


##################################################
#So let's dig into the DE results a little bit:

NACC_MainEffectOfEE<-read.delim("NAcc_remove2samples_covariatesocialdefeathemisphere_LR_allEE_vs_LR_allNil_deseq2_diff_raw.txt", sep="\t", header=T, stringsAsFactors = F)
str(NACC_MainEffectOfEE)
# 'data.frame':	15378 obs. of  10 variables:
#   $ gene            : chr  "ENSRNOG00000029897" "ENSRNOG00000014303" "ENSRNOG00000014330" "ENSRNOG00000049505" ...
# $ expression_level: num  13.7 8463.1 2759.7 107.7 1560.4 ...
# $ log_fold_change : num  -0.000815 0.01419 0.000283 -0.000462 -0.016028 ...
# $ p_value         : num  0.6437 0.1014 0.9604 0.847 0.0347 ...
# $ q_value         : num  0.923 0.623 0.991 0.971 0.485 ...
# $ stat            : num  -0.4626 1.6383 0.0496 -0.1929 -2.112 ...
# $ gene_id         : chr  "ENSRNOG00000029897" "ENSRNOG00000014303" "ENSRNOG00000014330" "ENSRNOG00000049505" ...
# $ gene_symbol     : chr  "AABR07000156.1" "Lrp11" "Pcmt1" "Nup43" ...
# $ gene_biotype    : chr  "protein_coding" "protein_coding" "protein_coding" "protein_coding" ...
# $ description     : chr  NA "LDL receptor related protein 11 [Source:RGD Symbol;Acc:1308602]" "protein-L-isoaspartate (D-aspartate) O-methyltransferase 1 [Source:RGD Symbol;Acc:3268]" "nucleoporin 43 [Source:RGD Symbol;Acc:1596513]" ...

colnames(NACC_MainEffectOfEE)<-paste("NACC_MainEffectOfEE", colnames(NACC_MainEffectOfEE))
colnames(NACC_MainEffectOfEE)[1]<-"gene"

NACC_MainEffectOfSD<-read.delim("NAcc_remove2samples_covariateenvironmenthemisphere_LR_allSD_vs_LR_allnoSD_deseq2_diff_raw.txt", sep="\t", header=T, stringsAsFactors = F)
str(NACC_MainEffectOfSD)
# 'data.frame':	16413 obs. of  10 variables:
#   $ gene            : chr  "ENSRNOG00000029897" "ENSRNOG00000014303" "ENSRNOG00000014330" "ENSRNOG00000049505" ...
# $ expression_level: num  13.7 8463.1 2759.7 107.7 1560.4 ...
# $ log_fold_change : num  -0.005 0.02814 0.01764 -0.00793 -0.02885 ...
# $ p_value         : num  0.213 0.115 0.162 0.316 0.07 ...
# $ q_value         : num  0.655 0.544 0.605 0.736 0.466 ...
# $ stat            : num  -1.24 1.57 1.4 -1 -1.81 ...
# $ gene_id         : chr  "ENSRNOG00000029897" "ENSRNOG00000014303" "ENSRNOG00000014330" "ENSRNOG00000049505" ...
# $ gene_symbol     : chr  "AABR07000156.1" "Lrp11" "Pcmt1" "Nup43" ...
# $ gene_biotype    : chr  "protein_coding" "protein_coding" "protein_coding" "protein_coding" ...
# $ description     : chr  NA "LDL receptor related protein 11 [Source:RGD Symbol;Acc:1308602]" "protein-L-isoaspartate (D-aspartate) O-methyltransferase 1 [Source:RGD Symbol;Acc:3268]" "nucleoporin 43 [Source:RGD Symbol;Acc:1596513]" ...

#Looks like same order but different number of genes (?)

colnames(NACC_MainEffectOfSD)<-paste("NACC_MainEffectOfSD", colnames(NACC_MainEffectOfSD))
colnames(NACC_MainEffectOfSD)[1]<-"gene"


NACC_EffectOfSD_StandardHousing<-read.delim("NAcc_remove2samples_covariatehemisphere_LR_Nil_SD_vs_LR_Nil_deseq2_diff_raw.txt", sep="\t", header=T, stringsAsFactors = F)

colnames(NACC_EffectOfSD_StandardHousing)<-paste("NACC_EffectOfSD_StandardHousing", colnames(NACC_EffectOfSD_StandardHousing))
colnames(NACC_EffectOfSD_StandardHousing)[1]<-"gene"

NACC_EffectOfSD_EEHousing<-read.delim("NAcc_remove2samples_covariatehemisphere_LR_EE_SD_vs_LR_EE_deseq2_diff_raw.txt", sep="\t", header=T, stringsAsFactors = F)

colnames(NACC_EffectOfSD_EEHousing)<-paste("NACC_EffectOfSD_EEHousing", colnames(NACC_EffectOfSD_EEHousing))
colnames(NACC_EffectOfSD_EEHousing)[1]<-"gene"

NACC_EffectOfEE_NoDefeat<-read.delim("NAcc_remove2samples_covariatehemisphere_LR_EE_vs_LR_Nil_deseq2_diff_raw.txt", sep="\t", header=T, stringsAsFactors = F)

colnames(NACC_EffectOfEE_NoDefeat)<-paste("NACC_EffectOfEE_NoDefeat", colnames(NACC_EffectOfEE_NoDefeat))
colnames(NACC_EffectOfEE_NoDefeat)[1]<-"gene"

NACC_EffectOfEE_SocialDefeated<-read.delim("NAcc_remove2samples_covariatehemisphere_LR_EE_SD_vs_LR_Nil_SD_deseq2_diff_raw.txt", sep="\t", header=T, stringsAsFactors = F)

colnames(NACC_EffectOfEE_SocialDefeated)<-paste("NACC_EffectOfEE_SocialDefeated", colnames(NACC_EffectOfEE_SocialDefeated))
colnames(NACC_EffectOfEE_SocialDefeated)[1]<-"gene"


NACC_EffectSummary<-join_all(list(NACC_MainEffectOfSD[,-c(7:10)], NACC_MainEffectOfEE[,-c(7:10)], NACC_EffectOfSD_StandardHousing[,-c(7:10)], NACC_EffectOfSD_EEHousing[,-c(7:10)], NACC_EffectOfEE_NoDefeat[,-c(7:10)], NACC_EffectOfEE_SocialDefeated), by="gene", type="inner")

str(NACC_EffectSummary)
colnames(NACC_EffectSummary)
colnames(NACC_EffectSummary)[c(32:35)]<-c("gene_id", "gene_symbol", "gene_biotype", "description")


sum(NACC_MainEffectOfSD$`NACC_MainEffectOfSD q_value`<0.10)
#[1] 129

sum(NACC_EffectOfSD_StandardHousing$`NACC_EffectOfSD_StandardHousing q_value`<0.10)
#[1] 63

sum(NACC_EffectOfSD_EEHousing$`NACC_EffectOfSD_EEHousing q_value`<0.10)
#[1] 17


sum(NACC_MainEffectOfEE$`NACC_MainEffectOfEE q_value`<0.10)
#[1] 43

sum(NACC_EffectOfEE_NoDefeat$`NACC_EffectOfEE_NoDefeat q_value`<0.10)
#[1] 17

sum(NACC_EffectOfEE_SocialDefeated$`NACC_EffectOfEE_SocialDefeated q_value`<0.10)
#[1] 1


#How do they compare in general?

plot(NACC_EffectSummary$`NACC_MainEffectOfSD log_fold_change`~NACC_EffectSummary$`NACC_MainEffectOfEE log_fold_change`)
#Ah, that's right - RNA-Seq data ends up making plus signs when you try to do an unbiased correlation.

plot(NACC_EffectSummary$`NACC_MainEffectOfSD log_fold_change`[NACC_EffectSummary$`NACC_MainEffectOfSD p_value`<0.001]~NACC_EffectSummary$`NACC_MainEffectOfEE log_fold_change`[NACC_EffectSummary$`NACC_MainEffectOfSD p_value`<0.001])
#No relationship between top SD genes and the effects of EE for those genes.

plot(NACC_EffectSummary$`NACC_MainEffectOfSD log_fold_change`[NACC_EffectSummary$`NACC_MainEffectOfEE p_value`<0.0005]~NACC_EffectSummary$`NACC_MainEffectOfEE log_fold_change`[NACC_EffectSummary$`NACC_MainEffectOfEE p_value`<0.0005])
#No relationship between the top EE genes and the effects of SD for those genes.

#What if we just look at under standard conditions?
plot(NACC_EffectSummary$`NACC_EffectOfSD_StandardHousing log_fold_change`[NACC_EffectSummary$`NACC_EffectOfSD_StandardHousing p_value`<0.001]~NACC_EffectSummary$`NACC_EffectOfEE_NoDefeat log_fold_change`[NACC_EffectSummary$`NACC_EffectOfSD_StandardHousing p_value`<0.001])
#There is a  handful of genes (21) that show an effect in both conditions that are positively correlated... but I should probably be careful when interpreting this because both comparisons use the same reference group (Nil_Standard Housing)

plot(NACC_EffectSummary$`NACC_EffectOfSD_StandardHousing log_fold_change`[NACC_EffectSummary$`NACC_EffectOfEE_NoDefeat p_value`<0.001]~NACC_EffectSummary$`NACC_EffectOfEE_NoDefeat log_fold_change`[NACC_EffectSummary$`NACC_EffectOfEE_NoDefeat p_value`<0.001])
#Similar result here.
...
NACC_EffectSummaryVsMetaAnalysis<-join(NACC_EffectSummary, MetaAnalysesResults_AllAges, by="gene_symbol", type="left")

setwd("~/Documents/Microarray Gen/Angela_HRLR_EE_Stress/NAcc_Remove2outliers")

write.csv(NACC_EffectSummaryVsMetaAnalysis, "NACC_EffectSummaryVsMetaAnalysis.csv")
